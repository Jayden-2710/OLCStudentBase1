...
for name in scores:
    print(f'{name} scored {score[name]})
...
for i in range(len(subjects)):
    subject_marks[subjects[i]]=marks[i]

### loop through dictionary to add all the values
subject["Math"] ## 75
subject["Science"] ## 82
subject["English"] ## 68
    
for sub in subject_marks:
    total = total + (subject_marks[sub])
...
while True:
    ...
    if len(username) <= 5:
        ...
        print('Please enter a username that is more then 5 characters')
...
if len(password)<8:
    print('please enter a password that has at least 8 characters')
...
# range check
while True:
    date= input('enter date in mm-yyyy format')
    # int(date[0,2]) = month
    # month = int(date[0:2])
    if month.isdigit(): ### .isdigit()
        if month>=1 and month<=9:
            int(date(3))=month # ???
            if month>=1900 and month<=2025:
                break
            else:
                continue
